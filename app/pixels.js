...
(function(root){
 ...
 function pot(c,id='pot-clay',x=22,y=42){const colors={'pot-clay':['#b76b48','#d39164','#e8ae80'],'pot-cloud':['#9cbfc2','#cee3de','#f0f4df'],'pot-berry':['#9d6584','#c58ba5','#e5b4c4'],'pot-night':['#3f4b70','#626e92','#8d99b6']},p=colors[id]||colors['pot-clay'];box(c,x,y,21,4,palette.ink);box(c,x+1,y+1,19,2,p[2]);box(c,x+2,y+4,17,13,palette.ink);box(c,x+3,y+4,15,11,p[1]);box(c,x+3,y+4,3,10,p[2]);box(c,x+15,y+4,3,11,p[0]);box(c,x+5,y+15,11,2,palette.ink);if(id==='pot-night'){box(c,x+4,y+6,1,2,'#f8d67d');box(c,x+3,y+7,3,1,'#f8d67d');}if(id==='pot-cloud'){box(c,x+5,y+12,4,1,'#edf4ec');box(c,x+6,y+11,2,1,'#edf4ec');}}
 ...
 function flower(c,{pot:potId='pot-clay',can:canId='can-mint',flower:type='flower-daisy',bend=0,sway=0,bob=0,frame=0,watering=false}={}){
 c.clearRect(0,0,64,64);box(c,19,60,28,2,'#31412e24');box(c,23,61,20,1,'#31412e16');const x=32+Math.round(13*bend+sway),y=16+Math.round(15*bend+bob),midX=32+Math.round(3*bend+sway/2),midY=29+Math.round(bob);
 ...
 if(watering){c.save();c.translate(42,0);c.scale(.62,.62);can(c,canId,32,false);c.restore();for(let n=0;n<5;n++){const dy=(frame*2+n*9)%32;box(c,24+n*4,8+dy,1,3,'#8ac1cf');box(c,24+n*4,8+dy,1,1,'#d5f0e8');}}
 }
 function can(c,id='can-mint',size=32,clear=true){if(clear)c.clearRect(0,0,size,size);const color={'can-mint':'#78a99c','can-honey':'#d4a557','can-rose':'#d59baa','can-sky':'#8ab7d1'}[id]||'#78a99c';box(c,9,13,13,12,palette.ink);box(c,10,14,11,10,color);box(c,11,15,3,8,'#ffffff55');box(c,12,10,8,3,palette.ink);box(c,14,8,4,2,palette.ink);box(c,22,14,5,3,palette.ink);box(c,25,15,3,8,palette.ink);box(c,22,22,5,3,palette.ink);line(c,9,19,4,12,palette.ink,4);line(c,9,18,5,12,color,2);box(c,2,10,6,3,palette.ink);box(c,1,9,2,4,color);box(c,2,15,1,3,'#8bc9d3');box(c,5,17,1,3,'#8bc9d3');}
