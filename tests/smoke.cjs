...
(async()=>{await main.ready;const pet=main.getPet();pet.webContents.on('console-message',(_event,details)=>{if(details.level==='error'){errors.push(details.message);console.log('CONSOLE ERROR',details.message);}});await pause(300);
 ...
 main.openPanel('settings');await pause(150);await panel.webContents.executeJavaScript('document.querySelector("#water-minutes").value="15";document.querySelector("#settings-form").requestSubmit()');await pause(200);assert.equal(main.getState().settings.waterMinutes,15);assert.equal('standMinutes' in main.getState().settings,false);
 ...
 main.openPanel('report');await pause(300);panel=main.getPanel();if(panel.webContents.isLoading())await new Promise(r=>panel.webContents.once('did-finish-load',r));assert.equal(await panel.webContents.executeJavaScript('document.querySelector("#report-answered").textContent'), '1');if(output)fs.writeFileSync(path.join(output,'report.png'),(await panel.webContents.capturePage()).toPNG());
 main.openPanel('welcome');await pause(200);if(output)fs.writeFileSync(path.join(output,'welcome.png'),(await panel.webContents.capturePage()).toPNG());
